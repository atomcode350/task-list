...
Manifests under `backend/k8s/` are assembled with Kustomize (`kustomization.yaml`). Runtime config is generated by `configMapGenerator` from `backend-config.env` (copied from `backend-config.env.example` and filled in CI), producing an immutable `backend-config-<hash>` ConfigMap. Kustomize rewrites the Deployment `envFrom` reference to that hashed name, so config changes roll out automatically and `kubectl rollout undo` restores the matching ConfigMap.

`aws-logging.yaml` is applied separately because it targets the `aws-observability` namespace, not the app namespace.

1. Placeholder resolution:
- `backend/k8s/kustomization.yaml` uses `#{K8S_NAMESPACE}#` (required so `configMapGenerator` and Deployments share a namespace and name references rewrite correctly)
- `backend/k8s/backend-config.env.example` uses `#{RDS_ENDPOINT}#`, `#{DB_NAME}#`, `#{AMPLIFY_DOMAIN}#`, `#{CF_DOMAIN}#`, `#{SECRETS_MANAGER_KEY}#`, `#{COGNITO_USER_POOL_ID}#`, `#{AWS_REGION}#`, and `#{SPRING_JPA_HIBERNATE_DDL_AUTO}#`
- `backend/k8s/aws-logging.yaml` uses `#{AWS_REGION}#` and `#{PROJECT_NAME}#`
- `backend/k8s/deployment.yaml` uses `#{IMAGE_URI}#`, `#{PROJECT_NAME}#`, and `#{K8S_NAMESPACE}#`
...
- `backend/k8s/deployment-debug.yaml` uses `#{IMAGE_URI}#`, `#{K8S_NAMESPACE}#`, `#{PROJECT_NAME}#`, and `#{BACKEND_CONFIGMAP_NAME}#`
- In CI/CD, `deploy-backend.yml` resolves these values automatically (infra values from SSM, image URI from build step)

2. Apply manifests:
```bash
# After resolving placeholders and writing backend/k8s/backend-config.env:
...
Use `backend/k8s/deployment-debug.yaml` to start a short-lived debug pod that reuses the live hashed backend ConfigMap and `ServiceAccount` but stays out of the main `Service` selector.
...
1. Connect `kubectl` to the cluster and resolve the current backend image and ConfigMap name:
```bash
...
sed -i.bak "s|#{BACKEND_CONFIGMAP_NAME}#|$BACKEND_CONFIGMAP_NAME|g" "$DEBUG_MANIFEST"
